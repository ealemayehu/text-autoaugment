...
def train_bert(
    ...
    if policy_opt:
        ...
        per_device_train_batch_size = int(
            config["per_device_train_batch_size"] / torch.cuda.device_count()
        )  # To make batch size equal to that in policy opt phase
    ...
    model = BertForSequenceClassification.from_pretrained(
        model_name_or_path, num_labels=get_num_class(dataset_type)
    )
...
def main():
    parser = argparse.ArgumentParser(description="Executes the text auto augment")
    parser.add_argument(
    ...
    input_directory = f"/home/ec2-user/research11/data/stage3/{args.dataset_name}/taa"

    output_directory = f"/home/ec2-user/research11/output/taa_{args.dataset_name}"
    os.makedirs(output_directory, exist_ok=True)

    # _ = C(f"{directory}/config.yaml")

    # search augmentation policy for specific dataset
    search_policy(
        dataset="custom_data",
        configfile=f"{input_directory}/config.yaml",
        abspath=output_directory,
    )

    data_files = {
        "train": f"{input_directory}/training.csv",
        "validation": f"{input_directory}/validation.csv",
        "test": f"{input_directory}/testing.csv",
    }
    ...
    augmented_train_dataset = augment(
        dataset=train_dataset, policy=policy, n_aug=8, configfile=configfile
    )
